...
def validate_native_symmetry_plane(
    ...
    *,
    ...
    meshes mirrored across YZ (X symmetry), XZ (Y symmetry), or both planes.
    Symmetry planes are not real BEM boundaries, so triangles lying entirely
    on a requested plane are rejected.
    """
    if symmetry_plane is None:
        return None
    plane = str(symmetry_plane).strip().lower()
    if plane not in {"yz", "xz", "yz+xz"}:
        raise MetalGeometryError(
            "native_symmetry_plane currently supports 'yz', 'xz', and 'yz+xz'"
        )
