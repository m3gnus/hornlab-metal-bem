...
class MetalNativeStandardSession:
    ...
    def assemble_solve_evaluate_standard_neumann_batch(
        ...
        *,
        ...
        expect_complex_k_per_case = [
            bool(k_imag != 0.0) for k_imag in k_imag_values
        ]
        # Complex-k and Robin capabilities are checked PER CASE: either can be
        # active on some frequencies and absent on others, so batch-global
        # expectations would falsely flag ordinary real-k/no-Robin cases as
        # coming from a stale helper binary at _dense_solve_field_result.
        ...
        if on_case_result is not None:
            return self._stream_assemble_solve_evaluate(
                ...
                expect_complex_k_per_case=expect_complex_k_per_case,
        ...
        return [
            self._dense_solve_field_result(
                case_result,
                batch_diagnostics,
                expect_complex_k=expect_complex_k_per_case[idx],
    ...
    def _stream_assemble_solve_evaluate(
        ...
        *,
        ...
        expected_count: int,
        on_case_result: Any,
        expect_complex_k_per_case: list[bool] | None = None,
        ...
        if expect_complex_k_per_case is None:
            expect_complex_k_per_case = [False] * expected_count
        ...
        def consume_available_cases() -> bool:
            ...
            while len(solved_fields) < expected_count:
                ...
                solved = self._dense_solve_field_result(
                    case_result,
                    {},
                    expect_complex_k=expect_complex_k_per_case[len(solved_fields)],
        ...
        for index in range(len(solved_fields), expected_count):
            solved = self._dense_solve_field_result(
                case_results[index],
                batch_diagnostics,
                expect_complex_k=expect_complex_k_per_case[index],
...
def _native_batch_diagnostics(result: dict[str, Any]) -> dict[str, Any]:
    keys = (
        ...
        "resident_context_device_seconds",
        "resident_context_library_seconds",
        "resident_context_pipeline_seconds",
        "resident_context_metal_library_source",
        "resident_context_metal_library_path",
        "resident_context_metal_library_cache_hit",
        "resident_context_pipeline_cache_hit",
