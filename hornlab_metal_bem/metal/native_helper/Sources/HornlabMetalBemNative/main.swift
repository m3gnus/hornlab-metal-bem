...
func evaluateCoupledIBRayleighReference(
    ...
    for obsIdx in observationPoints.indices {
        ...
        // The augmented coupling row assembles
        // p_aperture - 2*S_R*q_aperture = 0. Evaluate the exterior Rayleigh
        // field with the identical +2*S_R trace convention. Negating only this
        // field creates a 180-degree pressure discontinuity at the aperture even
        // though normalized directivity is unchanged.
        out[obsIdx] = acc * 2.0
    }
    return out
}
...
struct CircSymFieldKernelParams {
    var nTargets: Int32
    var nSources: Int32
    var nLine: Int32
    var nPsi: Int32
...
inline void accumulate_circsym_ring_point(
    thread float2 &slp,
    thread float2 &dlp,
    float targetRho,
    float targetZ,
    ...
    for (int p = 0; p < params.nPsi; ++p) {
        ...
        float r2 = targetRho * targetRho + sourceRho * sourceRho
            - 2.0f * targetRho * sourceRho * cp + dz * dz;
        if (r2 <= 1.0e-26f) {
            continue;
        }
        float r = sqrt(r2);
...
// Axisymmetric m=0 field integration. Each thread owns one target/source pair
// and integrates the line and azimuth quadrature locally, avoiding atomics and
// retaining enough independent work for typical 37 x N directivity fields.
kernel void evaluate_circsym_ring_kernels(
    ...
    constant CircSymFieldParams &params [[buffer(13)]],
    uint gid [[thread_position_in_grid]]
) {
...
func evaluateCircSymRingKernels(
    payloadPath: String,
    resultPath: String
) throws {
    let payload = try loadJSON(payloadPath)
    ...
    if try requireString(payload, "op") != "evaluate_circsym_ring_kernels" {
        try fail("expected evaluate_circsym_ring_kernels op")
    }
    ...
    let outputShape = [nTargets, nSources]
    ...
    let device = try MetalWarmup.shared.device()
    guard let commandQueue = device.makeCommandQueue() else {
        try fail("failed to create CircSym Metal command queue")
    }
    let libraryLoad = try assemblyLibrary(device: device)
    guard let function = libraryLoad.library.makeFunction(
        name: "evaluate_circsym_ring_kernels"
    ) else {
        try fail("failed to load resident Metal kernel evaluate_circsym_ring_kernels")
    }
    let pipeline = try device.makeComputePipelineState(function: function)
    let pairCount = nTargets * nSources
    let slpRe = try makeOutputBuffer(device, count: pairCount, label: "circsym_slp_re")
    let slpIm = try makeOutputBuffer(device, count: pairCount, label: "circsym_slp_im")
    let dlpRe = try makeOutputBuffer(device, count: pairCount, label: "circsym_dlp_re")
    let dlpIm = try makeOutputBuffer(device, count: pairCount, label: "circsym_dlp_im")
    let targetRho = try makeBuffer(device, targetRhoValues, label: "circsym_target_rho")
    let targetZ = try makeBuffer(device, targetZValues, label: "circsym_target_z")
    let sourceRho = try makeBuffer(device, sourceRhoValues, label: "circsym_source_rho")
    let sourceZ = try makeBuffer(device, sourceZValues, label: "circsym_source_z")
    let measure = try makeBuffer(device, measureValues, label: "circsym_measure")
    let normalRho = try makeBuffer(device, normalRhoValues, label: "circsym_normal_rho")
    let normalZ = try makeBuffer(device, normalZValues, label: "circsym_normal_z")
    ...
    guard let commandBuffer = commandQueue.makeCommandBuffer() else {
        try fail("failed to create CircSym Metal command buffer")
    }
    guard let encoder = commandBuffer.makeComputeCommandEncoder() else {
        try fail("failed to create CircSym Metal encoder")
    }
    encoder.setBuffer(slpRe, offset: 0, index: 0)
    encoder.setBuffer(slpIm, offset: 0, index: 1)
    encoder.setBuffer(dlpRe, offset: 0, index: 2)
    ...
    encoder.setBytes(
        &params,
        length: MemoryLayout<CircSymFieldKernelParams>.stride,
        index: 13
    )
    let dispatch = try dispatch1D(
        encoder: encoder,
    ...
    commandBuffer.waitUntilCompleted()
    if let error = commandBuffer.error {
        try fail("CircSym Metal field integration failed: \(error)")
    }

    try writeF32(
        try descriptorPath(root: root, descriptor: slpReDesc),
        readFloatBuffer(slpRe, count: pairCount)
    )
    try writeF32(
        try descriptorPath(root: root, descriptor: slpImDesc),
        readFloatBuffer(slpIm, count: pairCount)
    )
    try writeF32(
        try descriptorPath(root: root, descriptor: dlpReDesc),
        readFloatBuffer(dlpRe, count: pairCount)
    )
    try writeF32(
        try descriptorPath(root: root, descriptor: dlpImDesc),
        readFloatBuffer(dlpIm, count: pairCount)
    )
    try writeJSON(resultPath, [
        ...
        "field_seconds": CFAbsoluteTimeGetCurrent() - start,
...
func main(_ args: [String]) throws {
    ...
    } else if op == "evaluate_circsym_ring_kernels" {
        guard args.count == 3 else {
